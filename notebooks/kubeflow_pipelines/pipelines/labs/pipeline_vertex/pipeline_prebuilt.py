...
from bigquery_export_component import bigquery_export
...
def create_pipeline():
    ...
    bigquery_training_table = BigqueryQueryJobOp(
        project=PROJECT_ID,
        location="US",
        query="""
        CREATE OR REPLACE TABLE `covertype_dataset.training` AS
            SELECT *
    ...
    bigquery_validation_table = BigqueryQueryJobOp(
        project=PROJECT_ID,
        location="US",
        query="""
        CREATE OR REPLACE TABLE `covertype_dataset.validation` AS
            SELECT *
            FROM `covertype_dataset.covertype` AS cover
            WHERE
    ...
    bigquery_training_table_export = bigquery_export(
        table=bigquery_training_table.outputs["destination_table"],
        destination_uri=TRAINING_FILE_PATH,
        location="US",
    )

    bigquery_validation_table_export = bigquery_export(
        table=bigquery_validation_table.outputs["destination_table"],
    ...
    ).after(bigquery_training_table_export, bigquery_validation_table_export)
